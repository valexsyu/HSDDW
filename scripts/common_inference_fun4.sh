...
run_experiments() {
    ...
    local start_num_data=$6
    local datasets_name=("${@:7}")
    local root_path='/work/valex1377/LLMSpeculativeSampling/new_experiments'

    for dataset_name in "${datasets_name[@]}"
    do 
        # Call the function to set model names
        set_model_names $model $dataset_name $use_dy_gamma

        for i in $(seq $start_i $end_i)
        do
            echo "=========$dataset_name + $fn_name Start======${i}_${model}_topkp0_fp16_512tok${use_dy_gamma_suffix}==========="
            
            python main_modify.py \
                --input "The quick brown fox jumps over the lazy " \
                --file_root $root_path/${dataset_name}/${i}_${model}_topkp0_fp16_512tok${use_dy_gamma_suffix} \
                ...
                --max_tokens 512 \
                --load_bits 16 \
                --prefix_file_name ${i}_${model}_topkp0_fp16_512tok \
                $dataset_num_samples_arg \
                $model_70b_68m \
                $use_dy_gamma_arg \
            ...
            echo "=========$dataset_name END  =====${i}_${model}_topkp20_fp16_512tok${use_dy_gamma_suffix}==========="                    
        done
    done
}
