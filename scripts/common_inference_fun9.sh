# common_functions.sh
#from scripts/common_inference_fun7.sh, add topk=20 topp=0.9
...
set_model_names() {
    ...
    model_70b_68m=""
    if [[ "$model" == "70b_7b_68m" || "$model" == "70b_7b_68m_adv" || "$model" == "70b_7b_68m_pure" || "$model" == "70b_68m" || "$model" == "70b" ]]; then
        ...
        approx_model='llama-68m'     
    elif [[ "$model" == "68m" || "$model" == "68m_68m" || "$model" == "68m_68m_68m" || $model == "68m_7b_68m_adv" ]]; then
        target_model='llama-68m'
        approx_model='llama-68m'                          
    else
    ...
    if [ "$use_dy_gamma" = true ]; then
        ...
        if [ "$cal_entropy" = true ]; then
            cal_entropy_arg="--cal_entropy"
            use_dy_gamma_suffix="_record_only"
        else
            cal_entropy_arg=""
            use_dy_gamma_suffix="_record_only_pure"
        fi
    fi
...
run_experiments() {
    ...
    do 
        ...
        do
            echo "=========$dataset_name + $fn_name Start======${i}_${model}_topk20p09_temp${temperature}_fp16_2048tok${use_dy_gamma_suffix}==========="
            
            python main_modify.py \
                --input "The quick brown fox jumps over the lazy " \
                --file_root $root_path/${dataset_name}/sp_dy_gamma_${i}_${model}_topk20p09_temp${temperature}_fp16_2048tok${use_dy_gamma_suffix} \
            ...
            echo "=========$dataset_name END  =====${i}_${model}_topk0p0_fp16_2048tok${use_dy_gamma_suffix}==========="                    
        done
    done
}
