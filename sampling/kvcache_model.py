...
class KVCacheModelEntropyGamma(KVCacheModel):
    def __init__(self,approx_model_1, temperature, top_k, top_p,use_dy_gamma, cal_entropy):
        super().__init__(approx_model_1, temperature, top_k, top_p)
        self.cal_entropy = cal_entropy
        self.use_dy_gamma=use_dy_gamma
    ...
    def _generate_with_kvcache(self, prefix : torch.Tensor, 
        ...
        for i in range(gamma):
            ...
            if self.cal_entropy :
                ...
                if entropy > entropy_th  and self.use_dy_gamma :
                    break 
...
class KVCacheModelEntropyHrchlGamma(KVCacheModelEntropyGamma):
    def _generate_with_kvcache(self, prefix : torch.Tensor, 
                                    gamma : int, entropy_th : float,
                                    use_debug = False) -> torch.Tensor:
        """ forward the model gamma times
        ...
        entropys=[]
        for i in range(gamma):
            q, not_cached_q = self._forward_with_kvcache(x, use_debug)
            next_tok = sample(q)
            x = torch.cat((x, next_tok), dim=1)
    ...
    def _forward_with_kvcache(self, input_ids : torch.Tensor, use_debug = True) -> torch.Tensor:
        if self._past_key_values is None:
            ...
            self._past_key_values = outputs.past_key_values
            last_q = self._prob_history[:, -1, :]
            not_cached_q = self._prob_history 
        else:
            # return the last token's logits
            cached_len = 0
            for kv in self._past_key_values:
        ...
        return last_q, outputs.logits      
    


class KVCacheModelEntropyHrchlAdvGamma(KVCacheModelEntropyHrchlGamma):
    @torch.no_grad()
    def generate(self, input : torch.Tensor, gamma : int, entropy_th : float, pred_len:int) -> torch.Tensor:
        output, gamma_out, entropys = self._generate_with_kvcache(input, gamma, entropy_th,pred_len)
        return output, gamma_out ,entropys    
    def _generate_with_kvcache(self, prefix : torch.Tensor, 
                                    gamma : int, entropy_th : float,
                                    pred_len : int,
        ...
        next_tok = sample(q)
        x = torch.cat((x, next_tok), dim=1)
        distribution = torch.distributions.Categorical(probs=not_cached_q[:,-(pred_len+1):,:])
        entropys = distribution.entropy().tolist()[0]
